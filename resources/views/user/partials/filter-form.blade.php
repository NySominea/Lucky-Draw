<form method="GET" action="{{ route('users.index') }}" class="form-inline mb-sm-2 d-block d-sm-flex" id="form-filter">
    ...
    <div class="form-group mr-0 mr-sm-2">
        <select class="custom-select" id="role-select" name="roles_id">
            <option value="">Role</option>
            @foreach ($roles as $id => $role)
            <option value="{{ $id }}" {{ (int) request()->roles_id === $id ? 'selected' : '' }}>{{ $role }}</option>
            @endforeach
        </select>
    </div>
